...
### 1.4 FlashAI 操作逻辑参考边界

本项目可以参考本机 FlashAI 视频处理功能中已经验证过的通用操作逻辑，
但不复制它的品牌、视觉素材、接口实现或云端上传方式。

采用的交互原则：

1. “视频任务”是主要工作区，创建入口、历史列表和当前详情保持在同一页面。
2. 一个视频项目在列表中只显示一个条目；导入、识别、翻译和导出作为右侧详情中的阶段展示。
3. 创建任务时集中确认视频、处理方式、源语言、目标语言、导出模式和识别运行配置。
4. 任务列表来自 SQLite 持久化状态，应用重启后仍可查看源文件、阶段、进度、错误和恢复检查点。
5. 创建后自动进入任务详情，长任务持续展示进度和逐句译文；完成后不把用户强制带离任务工作区。

保持的产品差异：

1. Zero Caption 直接读取用户选择的本地路径，不把视频包装成网页上传请求。
2. ASR 模型、CPU/GPU 和推理精度在设置页统一配置，创建任务时只显示本次实际配置，避免同一能力出现两套冲突入口。
3. 云端仍只接收字幕文本和必要上下文，视频、音频、截图和项目文件都留在本机。
4. 当前单任务版本不伪装支持“暂停全部”；只有持久化 worker 真正接管执行后才增加批量暂停和恢复操作。
